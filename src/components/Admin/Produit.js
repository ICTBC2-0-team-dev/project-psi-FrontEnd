...
const Produit = () => {
    ...
    const handleValiderClick = async () => {
        ...
        if (selectedId) {
            try {
                const request = await axios.put(`/article/valide/${selectedId}`, {
                    id: selectedId
                });
                // Traitement de la réponse de la requête
                console.log('Article accepter avec success', request.data);
    ...
    const handleRejeterrClick = () => {
        ...
        if (selectedId) {
            ...
            axios.put(`/article/rejette/${selectedId}`)
                .then(response => {
                    // Traitement de la réponse de la requête
                    console.log('Article Rejeter avec success', response.data);
    ...
    useEffect(() => {
        const fectData = async () => {
            try {
                const request = await axios.get('/articlesRejette');
                setArticlearticlesRejette(request.data);
                console.log(request.data);
            } catch (error) {
                console.log(error);
            }
                                            ...
                                            <Card sx={{ maxWidth: 345 }}>
                                                <Box>
                                                    ...
                                                    <CardMedia
                                                        ...
                                                        image={`http://localhost:8081/${article.photo}`}
                                                    />
                                                </Box>
                                                <CardContent>
