...
const DetailProjet = () => {
  ...
  const profilUserId = getProfilUserId();
  ...
  React.useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await axios.get(`http://localhost:8081/modulesByProject/${id}`);
  ...
  /** recuperation des competence */
  // const [competences, setCompetences] = React.useState([]);
  // const [selectedCompetence, setSelectedCompetence] = React.useState("");
